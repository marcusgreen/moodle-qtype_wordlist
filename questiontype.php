...
/**
 * Question type class for the wordlist question type.
 *
 * @package    qtype
 * @subpackage wordlist
...
require_once($CFG->dirroot . '/question/type/wordlist/question.php');


/**
 * The wordlist question type.
 *
 * @copyright  THEYEAR YOURNAME (YOURCONTACTINFO)

 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_wordlist extends question_type {

      /* ties additional table fields to the database */
    public function extra_question_fields() {
        return array('question_wordlist', 'somefieldname','anotherfieldname');
    }
    ...
    public function save_question_options($question) {
        global $DB;
        $options = $DB->get_record('question_wordlist', array('questionid' => $question->id));
        if (!$options) {
        ...
        $DB->update_record('question_wordlist', $options);
 ...
 **/
   ...
   public function initialise_question_answers(question_definition $question, $questiondata,$forceplaintextanswers = true){
    ...
    public function import_from_xml($data, $question, qformat_xml $format, $extra = null) {
        if (!isset($data['@']['type']) || $data['@']['type'] != 'question_wordlist') {
            return false;
        }
    ...
    public function export_to_xml($question, qformat_xml $format, $extra = null) {
        ...
        $gapfillinfo = $pluginmanager->get_plugin_info('question_wordlist');
